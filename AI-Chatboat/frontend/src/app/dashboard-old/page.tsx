...
export default function Page() {
  ...
  useEffect(() => {
    if (!showWelcome) {
      // Remove this line - it was resetting the tour
      // setTimeout(() => setWalkthroughStep(0), 500);
    }
  }, [showWelcome]);

  const handleCloseWelcome = () => {
    setShowWelcome(false);
    setWalkthroughStep(1); // Start the tour at step 1
    localStorage.setItem("welcomeSeen", "true");
  };   
  

  const finishTour = () => {
    setWalkthroughStep(-1); // or null if your logic supports it
    setShowWelcome(false);
